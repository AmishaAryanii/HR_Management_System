...
const authLimiter = rateLimit({
  ...
  max: 20, // 20 failed login attempts per window
...
const swaggerOptions = {
  definition: {
    ...
    info: {
      ...
      contact: { name: 'HRMS Team', email: 'support@hrms.com' }
    },
    servers: [
    ...
    server.on('error', (err) => {
      if (err.code === 'EADDRINUSE') {
        ...
        if (port < DEFAULT_PORT + 10) {
          ...
          console.error('❌ All nearby ports are in use. Please stop the other process or set a different PORT in .env');
          process.exit(1);
        }
      } else {
