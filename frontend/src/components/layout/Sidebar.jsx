...
import {
  HiOutlineHome, HiOutlineUsers, HiOutlineOfficeBuilding, HiOutlineIdentification,
  HiOutlineCalendar, HiOutlinePaperAirplane, HiOutlineCash, HiOutlineClipboardList,
  HiOutlineBriefcase, HiOutlineChartBar, HiOutlineCheckCircle, HiOutlineSpeakerphone,
  HiOutlineDocumentText, HiOutlineCog, HiOutlineChevronLeft, HiOutlineX,
    ...
    <div className="flex flex-col h-full">
          ...
          <div className="w-10 h-10 rounded-xl flex items-center justify-center overflow-hidden flex-shrink-0 shadow-soft">
            {company?.logo ? (
              ...
              <div className="w-10 h-10 bg-gradient-to-br from-primary-500 to-primary-700 rounded-xl flex items-center justify-center">
                <span className="text-white font-bold text-sm font-display">{company?.companyName?.charAt(0) || 'H'}</span>
              </div>
            )}
          </div>
          {sidebarOpen && (
            <div className="min-w-0 animate-fade-in">
              <p className="text-sm font-semibold text-secondary-900 truncate">{company?.companyName || 'Debox Technology'}</p>
              <p className="text-xs text-secondary-400 truncate">{company?.email || ''}</p>
            </div>
          )}
        </div>
      ...
      {sidebarOpen && (
        <div className="px-4 py-3.5 border-b border-gray-50">
          ...
          <p className="text-xs text-secondary-400 capitalize">{userRole?.replace('_', ' ')}</p>
        </div>
      )}
